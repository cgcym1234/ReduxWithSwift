...
extension Array where Element: Comparable {
    ...
    public func minMaxValueIndex(at starIndex: Int = 0) -> (min: Int, max: Int)? {
    ...
    public func maxValueIndex(at startIndex: Int = 0) -> Int? {
        return minMaxValueIndex(at: startIndex)?.max
    }
    public func minValueIndex(at startIndex: Int = 0) -> Int? {
        return minMaxValueIndex(at: startIndex)?.min
    }
}
...
extension Array where Element: Comparable {
    ...
    mutating func quickSortImprove(lowIndex: Int, highIndex: Int, k: Int) {
        ...
        if highIndex - lowIndex > k {
            // 将表一分为二
            let pivotIndex = partition(lowIndex: lowIndex, highIndex: highIndex)
            
            // 递归对低子表递归排序
            quickSortImprove(lowIndex: lowIndex, highIndex: pivotIndex - 1, k: k)
            
            // 递归对高子表递归排序
            quickSortImprove(lowIndex: pivotIndex + 1, highIndex: highIndex, k: k)
        }
    }
    mutating func quickSortBetter() {
        ...
        // 先调用改进算法，使之基本有序
        quickSortImprove(lowIndex: 0, highIndex: count - 1, k: 8)
